...
Muscle::muscle_state_t Muscle::updateMuscle(Muscle::muscle_cmd_t muscle_cmd) 
{
  ...
  {
    board_->setOutputNormalized(dac_index_, muscle_cmd.goal_activation); //Muscleクラスのメンバー関数であるupdateMuscleは、同じクラスのインスタンスのprivateメンバー変数やメンバー関数にアクセスする権限を持っています。
...
Muscle::muscle_state_t Muscle::printMuscle(Muscle::muscle_cmd_t muscle_cmd)
{
  ...
  //std::cout << "model length is " << board_->getLoadCellData(tension_sensor_index_) << std::endl;
