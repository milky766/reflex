...
//change mode : 1
    ...
    */
    ...
    //追加
    double *voltage_data_left = new double[100000];
    double *voltage_data_right = new double[100000];
...
//change mode: 2 //タスク
        ...
        reflex_commands_agonist = low_con->agonist_Ia_innervation(); //この4行何しているかわからないので確認
        ...
        */
        switch(0)  //反射 Yanlinさんのはもっとシンプルな伸長反射のみできるはず
        {
            case 0: 
            {
...
// change mode: 3 以下のブロックでは、各センサーのデータを記録し、ファイルに書き込むための準備を行います。
        ...
        //记录数据：
        tension_data_left[sample_count] = sensor_info_agonist.muscle_tension; //変更 モデル値を習得
        ms_data_left[sample_count] = sensor_info_agonist.muscle_len;//muscle_lenを取得するコードを変更
        ...
        //追加
        // voltage_data_left[sample_count] = ;
        // voltage_data_right[sample_count] = ;
        // ms_data_left_model[sample_count] = ;
        // ms_data_right_model[sample_count] = ;
        // ms_data_left_speed_model[sample_count] = ;
        // ms_data_right_speed_model[sample_count] = ;
...
//        usleep(10000); //10000 us = 10 ms -> 100 Hz
    ...
    delete[] voltage_data_left;
    delete[] voltage_data_right;
